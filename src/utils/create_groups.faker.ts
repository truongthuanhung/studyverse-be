...
const createGroupInvitation = async (group_id: ObjectId, created_user_id: ObjectId, invited_user_id: ObjectId) => {
...
const createStudyGroups = async () => {
  ...
  console.log(
    `Đã tạo thành công ${GROUP_COUNT} nhóm học và gửi lời mời tham gia cho người dùng ${TARGET_ID.toString()}`
  );
};
